...
public class WallRunState : MovementState
{
    ...
    [Space(15)] [Header("Custom CameraProfile")] [SerializeField]
    CameraProfile _wallRunProfile;
    ...
    public override void UpdateState()
    {
        ...
        _cameraController.UpdateWallRunCamera(wallHit.normal, _meshModel.transform.forward, _meshModel.transform.up);
    ...
    private void SnapModel(RaycastHit wallHit)
    {
        ...
        if (Vector3.Dot(wallRunDirection, _meshModel.transform.forward) < 0f) wallRunDirection = -wallRunDirection;

        // Make it look the wall run direction
        _meshModel.transform.rotation = Quaternion.LookRotation(wallRunDirection, Vector3.up);
        // Accelerate the player along the wall
        acceleration =
            wallRunDirection * (_controller.VerticalInput * _accelerationRate) +
